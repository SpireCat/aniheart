var styleBuffer = '';
var fullTextStorage = {};  //用于存储fulltext
...
module.exports = function writeChar(el, char, style){
  ...
  var fullText = fullTextStorage[el.id];  //fulltext为el的全部文本
  if (!fullText) fullText = fullTextStorage[el.id] = el.innerHTML;  // 如果为空，则将el的全部文本赋给fulltext

  fullText = module.exports.handleChar(fullText, char);  //根据char判断要进行的动作
  // But we still write to the DOM every iteration, which can be pretty slow.
  el.innerHTML = fullTextStorage[el.id] = fullText;    //将fullText写回el中

  // Buffer writes to the <style> element so we don't have to paint quite so much.
  styleBuffer += char;   
  if (char === ';') {
    style.textContent += styleBuffer;    //如果char为;那么将Buffer中的内容写入到样式style的文本中，并清空这个缓冲区
    styleBuffer = '';   
  }
};

module.exports.simple = function writeSimpleChar(el, char) {
  el.innerHTML += char;      //仅将元素中的HTML添加
};
